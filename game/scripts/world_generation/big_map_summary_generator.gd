...
func generate(config, skeleton):
	...
	for row in range(config.big_map_size):
		for col in range(config.big_map_size):
			var tile = _generate_tile_summary(config, skeleton, sampler, col, row)
...
func _generate_tile_summary(config, skeleton, sampler, col: int, row: int):
	...
	for sy in range(resolution):
		for sx in range(resolution):
			...
			var height: int = sampler.sample_height(world_x, world_y)
			var biome: String = sampler.sample_biome(world_x, world_y)
	...
	tile.has_river = tile.river_strength > 0.08 or skeleton.rivers_by_tile.has(tile.tile_key)
