...
河流从山脉影响范围内的高海拔陆地源头出发，按大地图 8 邻域逐步选择更低海拔地块。到达海洋后停止；进入已有河流宽度范围后停止并视为汇流；无更低相邻地块时记录湖泊。河流宽度单位为全局地格，默认从 `3` 随距离和汇流增长到最大 `12`。

下一阶段河流不应只停留在大地图路径。大地图路径保留为粗路径，负责预览、摘要和索引；小地图第一次进入时根据粗路径进行地格级细化。细化后的河流会保存沿线切割点、宽度和深度，使河流能通过切割低成本地形继续前进，更容易入海。
