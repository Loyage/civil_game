...
- 大地图海拔后续改为 `-256..256` 整数范围
- 小地图地格高度使用 `-256..256` 整数范围
- 大地图地块海拔后续需要能由局部地图高度统计校验或调整
...
- 如果大地图海拔立刻改为小地图平均值，会导致当前大地图生成逻辑大幅重构；首版采用兼容策略，先让小地图匹配大地图宏观信息，后续再反向校验或调整。
- 小地图地格数量为 `sub_map_size * sub_map_size`。如果每个地格都存完整对象，内存和存档体积会快速膨胀；首版需要优先考虑紧凑数组结构。
- JSON 对完整高度数组不适合长期使用，缓存格式需要尽早规划成二进制或压缩格式。
