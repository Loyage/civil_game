...
`local_map` 模块负责生成和展示大地图单个地块内部的局部地图。大地图中的每个地块 `tile`，进入后对应一个 `256 x 256` 的小地图；小地图中的每个单位称为地格 `cell`。

首版目标：

- 每个大地图地块都可以按需生成一个 `256 x 256` 小地图
...
- `256 x 256 = 65536` 个地格，如果每个地格都存完整对象，内存和存档体积会快速膨胀；首版需要优先考虑紧凑数组结构。
