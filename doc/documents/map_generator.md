...
- 小地图预览：
  ...
  - 小地图“特征”视图按 `terrain_flags` 显示草地、森林、湿地、岩石、沙地和雪地。
  - 左侧面板显示 tile key、平均高度和选中地格信息。

- 地图预览视觉增强：
  - 预览工具保留现有颜色视图作为主信息，并在地貌类视图中叠加低对比纹理和紧凑图标。
  - “视觉增强”控制支持“自动”和“关闭”；默认“自动”。
  - 自动模式在基础地貌、特征和资源视图启用，在海拔、湿度、温度、河流和走向视图保持纯色或现有 overlay。
  - 大地图和小地图都使用同一套符号语义：资源、山地/岩石、森林、湿地、沙地/雪地和丘陵。
  - 河流继续使用现有蓝色路径或色带，不用图标替代。
  - 左侧摘要会在视觉增强生效时显示简短图例。
  - 首版不引入外部贴图资源，增强生效时会提高预览源图的每格像素数，再在预览 `Image` 上绘制程序化像素纹理和符号，最终仍缩放到原有显示尺寸。
...
| `resources` | 在环境地貌后按资源定义生成动物/矿产类型列表。 | 生成到资源步骤后的整体样貌。 |
...
- `width`
- `height`
- 每个地块的 `biome`、高度摘要、温度、湿度、河流信息、路径点、`terrain_tags` 和 `resource_ids`
