...
- 小地图资源使用 `resource_instances` 表示具体地格实例，由大地图 `resource_ids` 派生。
...
`resource_instances` 在地貌标签生成之后派生：

- 动物资源必须落在资源定义允许的地貌地格上，例如牛/马偏草地，老虎偏森林。
- 动物实例带 `instance_id`，当前静止不动，预留给未来移动和保存系统。
- 矿产资源优先落在岩石、沙地等定义允许地貌上；找不到合格地格时退化到任意非水陆地。
- 矿产会生成 `2..5` 个相邻或近邻地格，形成小矿脉。
- 资源实例当前不写入小地图缓存，缓存读取后会按同 seed 和大地图资源列表重新派生。
...
“资源”视图会按资源定义颜色显示动物和矿产地格；选中地格时，左侧信息会显示资源中文名。
...
- 湖泊和植被暂未作为独立小地图对象细化；资源已有静态实例，但尚无移动、采集或产出逻辑。
